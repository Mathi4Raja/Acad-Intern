...
export function TestimonialsSection() {
    ...
    return (
        <section id="testimonials" ref={ref} className="py-12 md:py-24 lg:py-32 bg-muted/40 relative overflow-hidden">
...
function TestimonialCard({ t, compact = false }: { t: typeof testimonials[0]; compact?: boolean }) {
    return (
        <div className={`h-full ${compact ? "p-4" : "p-5 md:p-6"} rounded-xl md:rounded-2xl border border-border/60 bg-card/80 backdrop-blur-sm card-hover`}>
            {/* Stars */}
            <div className="flex gap-0.5 mb-2 md:mb-4">
                {[...Array(5)].map((_, i) => (
                    <svg key={i} className={`${compact ? "w-3.5 h-3.5" : "w-4 h-4 md:w-5 md:h-5"} text-yellow-400`} fill="currentColor" viewBox="0 0 20 20">
            ...
            {/* Quote */}
            <p className={`text-muted-foreground leading-relaxed ${compact ? "text-sm mb-3" : "text-sm md:text-base mb-4 md:mb-6"}`}>
                &ldquo;{t.text}&rdquo;
            </p>

            {/* Author */}
            <div className="flex items-center gap-2 md:gap-3">
                <div className={`${compact ? "w-8 h-8" : "w-9 h-9 md:w-11 md:h-11"} rounded-full bg-gradient-to-br ${t.gradient} flex items-center justify-center text-white font-semibold text-xs md:text-sm shadow-lg`}>
                    {t.avatar}
                </div>
                <div>
                    <p className={`font-semibold ${compact ? "text-sm" : "text-sm md:text-base"}`}>{t.name}</p>
                    <p className={`text-muted-foreground ${compact ? "text-xs" : "text-xs md:text-sm"}`}>{t.role}</p>
                </div>
            </div>
        </div>
    );
